...
fn build_part_child_field_decl(child: &PartChild, all_parts: &[Part]) -> PartFieldDecl {
  ...
  PartFieldDecl {
    ...
    kind: PartFieldKind::Child {
      relationship_type,
      cardinality: if child.max_occurs_great_than_one {
...
mod tests {
  ...
  fn builds_part_codegen_ir_with_explicit_child_relationship_type() {
    ...
    assert!(matches!(ir.fields[3].kind, PartFieldKind::InnerPath));
    assert!(matches!(ir.fields[4].kind, PartFieldKind::RootElement));
    assert!(matches!(
      &ir.fields[5].kind,
      PartFieldKind::Child {
        relationship_type,
