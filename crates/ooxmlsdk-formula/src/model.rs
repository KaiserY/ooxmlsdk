...
fn round_direction(value: f64, digits: i32, away_from_zero: bool) -> f64 {
  ...
  const LO_ROUND_SIG_DIGITS: i32 = 12;
  if digits < LO_ROUND_SIG_DIGITS && value.fract() != 0.0 {
    let temp = value.abs().log10().floor() as i32 + 1 - LO_ROUND_SIG_DIGITS;
...
fn normalized_date_components(year: i32, month: i32, day: i32) -> Option<(i32, u32, u32)> {
...
fn is_valid_libreoffice_gregorian_date(year: i32, month: u32, day: u32) -> bool {
...
fn lo_poisson_dist(x: f64, lambda: f64, cumulative: bool) -> f64 {
  if !cumulative {
    if lambda > 712.0 {
...
fn parse_formula_error_literal_at(value: &str, start: usize) -> Option<(&str, usize)> {
  ...
  formula_error_literals()
    ...
    .find(|literal| {
      rest
        .get(..literal.len())
        .is_some_and(|prefix| prefix == **literal)
    })
