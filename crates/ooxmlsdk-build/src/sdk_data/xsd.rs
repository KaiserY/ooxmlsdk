...
pub(crate) struct ParsedNamedGroups {
  pub target_namespace: String,
  pub groups: BTreeMap<String, ParsedGroupParticle>,
}

#[derive(Clone, Debug)]
pub(crate) enum ParsedGroupParticle {
  Element(String),
  GroupRef(String),
  Choice(Vec<ParsedGroupParticle>),
  Sequence(Vec<ParsedGroupParticle>),
}
...
pub(crate) fn parse_named_groups(source: &str, default_prefix: &str) -> Result<ParsedNamedGroups> {
  let mut reader = Reader::from_str(source);
  reader.config_mut().trim_text(true);
  let mut parsed = ParsedNamedGroups::default();
      ...
      Event::Start(e) => match local_name(e.name().as_ref()) {
        ...
        b"group" => {
          if let Some(name) = optional_attr(&reader, &e, b"name")? {
            let group_name = normalize_prefixed_name(name.as_str(), default_prefix);
            if let Some(particle) = parse_named_group_body(&mut reader, e, default_prefix)? {
              parsed.groups.insert(group_name, particle);
            }
          } else {
...
pub(crate) fn resolve_named_group_leaf_sets(
  groups: &BTreeMap<String, ParsedGroupParticle>,
) -> BTreeMap<String, std::collections::BTreeSet<String>> {
  fn collect_leafs(
    group_name: &str,
    groups: &BTreeMap<String, ParsedGroupParticle>,
    resolving: &mut std::collections::BTreeSet<String>,
    cache: &mut BTreeMap<String, std::collections::BTreeSet<String>>,
  ) -> std::collections::BTreeSet<String> {
    if let Some(cached) = cache.get(group_name) {
      return cached.clone();
    }

    if !resolving.insert(group_name.to_string()) {
      return std::collections::BTreeSet::new();
    }

    let mut leafs = std::collections::BTreeSet::new();

    if let Some(particle) = groups.get(group_name) {
      collect_group_particle_leafs(particle, groups, resolving, cache, &mut leafs);
    }

    resolving.remove(group_name);
    cache.insert(group_name.to_string(), leafs.clone());
    leafs
  }
  ...
  for group_name in groups.keys() {
    ...
    collect_leafs(group_name, groups, &mut resolving, &mut cache);
  }

  cache
}

fn collect_group_particle_leafs(
  particle: &ParsedGroupParticle,
  groups: &BTreeMap<String, ParsedGroupParticle>,
  resolving: &mut std::collections::BTreeSet<String>,
  cache: &mut BTreeMap<String, std::collections::BTreeSet<String>>,
  ...
  match particle {
    ParsedGroupParticle::Element(q_name) => {
      out.insert(q_name.clone());
    }
    ParsedGroupParticle::GroupRef(group_name) => {
      if let Some(cached) = cache.get(group_name) {
        out.extend(cached.iter().cloned());
        return;
      }

      if !resolving.insert(group_name.clone()) {
        return;
      }

      if let Some(group_particle) = groups.get(group_name) {
        collect_group_particle_leafs(group_particle, groups, resolving, cache, out);
      }

      resolving.remove(group_name);
    }
    ParsedGroupParticle::Choice(children) | ParsedGroupParticle::Sequence(children) => {
      for child in children {
        collect_group_particle_leafs(child, groups, resolving, cache, out);
      }
    }
  }
}

fn parse_named_group_body(
  reader: &mut Reader<&[u8]>,
  _start: BytesStart<'_>,
  default_prefix: &str,
) -> Result<Option<ParsedGroupParticle>> {
  let mut particle = None;

  loop {
    match reader.read_event()? {
      Event::Start(e) => match local_name(e.name().as_ref()) {
        b"choice" | b"sequence" | b"all" => {
          particle = Some(parse_group_container(reader, e, default_prefix)?);
        }
        _ => skip_element(reader, e.name().as_ref())?,
      },
      Event::Empty(e) => match local_name(e.name().as_ref()) {
        b"choice" => particle = Some(ParsedGroupParticle::Choice(Vec::new())),
        b"sequence" | b"all" => particle = Some(ParsedGroupParticle::Sequence(Vec::new())),
        _ => {}
      },
      Event::End(e) if local_name(e.name().as_ref()) == b"group" => break,
      Event::Text(_) | Event::Comment(_) => {}
      Event::Eof => return Err("unexpected EOF in named group".into()),
      _ => {}
    }
  }
...
fn parse_group_container(
  reader: &mut Reader<&[u8]>,
  start: BytesStart<'_>,
  default_prefix: &str,
) -> Result<ParsedGroupParticle> {
  let mut children = Vec::new();
  let is_choice = local_name(start.name().as_ref()) == b"choice";

  loop {
    match reader.read_event()? {
      Event::Start(e) => match local_name(e.name().as_ref()) {
        b"choice" | b"sequence" | b"all" => {
          children.push(parse_group_container(reader, e, default_prefix)?);
        }
        b"element" => {
          children.push(parse_group_element(reader, &e, default_prefix)?);
          skip_element(reader, e.name().as_ref())?;
        }
        b"group" => {
          children.push(parse_group_reference(reader, &e, default_prefix)?);
          skip_element(reader, e.name().as_ref())?;
        }
        _ => skip_element(reader, e.name().as_ref())?,
      },
      Event::Empty(e) => match local_name(e.name().as_ref()) {
        b"element" => children.push(parse_group_element(reader, &e, default_prefix)?),
        b"group" => children.push(parse_group_reference(reader, &e, default_prefix)?),
        b"choice" => children.push(ParsedGroupParticle::Choice(Vec::new())),
        b"sequence" | b"all" => children.push(ParsedGroupParticle::Sequence(Vec::new())),
        _ => {}
      },
      Event::End(e) if e.name().as_ref() == start.name().as_ref() => break,
      Event::Text(_) | Event::Comment(_) => {}
      Event::Eof => return Err("unexpected EOF in group container".into()),
      _ => {}
    }
  }

  Ok(if is_choice {
    ParsedGroupParticle::Choice(children)
  } else {
    ParsedGroupParticle::Sequence(children)
  })
}
...
fn parse_group_element(
  ...
  let q_name = if let Some(reference) = optional_attr(reader, element, b"ref")? {
    normalize_prefixed_name(reference.as_str(), default_prefix)
  } else {
    let name = required_attr(reader, element, b"name")?;
    normalize_prefixed_name(name.as_str(), default_prefix)
  };

  Ok(ParsedGroupParticle::Element(q_name))
}
...
fn parse_group_reference(
  ...
  let group_name = required_attr(reader, element, b"ref")?;
  Ok(ParsedGroupParticle::GroupRef(normalize_prefixed_name(
    group_name.as_str(),
    default_prefix,
  )))
}

fn normalize_prefixed_name(value: &str, default_prefix: &str) -> String {
  if value.contains(':') || default_prefix.is_empty() {
    value.to_string()
  } else {
