...
use crate::docx::{
  ...
  FloatingFrame, FloatingFramePlacement, FloatingImagePlacement, FloatingPaintOrder,
  FrameHeightRule, FrameHorizontalAlignment, FrameHorizontalAnchor, FrameVerticalAlignment,
  ...
  InlineShape, InlineShapeGeometry, InlineShapeImageFill, InlineShapeImageFillMode,
  LegacyTextRelief, LineHeightRule, LineNumbering, NoteNumberingSpec, PRESERVED_WORD_TEXT_TAB,
  PageBottomHyphenation, PageSetup, ParagraphAlignment, PositionalTab, PositionalTabBase, RgbColor,
  RubyAlignment, RubyInline, SectionBreakKind, SectionColumns, TabLeader, TabStop,
...
// Inline endnotes keep 269 twips below the separator.
...
fn inline_drawing_line_height(
  ...
  text_frame: TextFrame,
  text_metrics: &mut TextMetrics,
  uses_legacy_chart_only_line: bool,
) -> f32 {
...
fn inline_drawing_top(
  ...
  let baseline =
    if text_frame.compatibility_mode < 15 && paragraph_has_only_inline_drawing_content(paragraph) {
      // SettingsTable enables MS_WORD_COMP_MIN_LINE_HEIGHT_BY_FLY through
      // compatibility mode 14. For an inline drawing followed only by the
      // implicit paragraph mark, Writer keeps that mark's natural line box as
      // the ascent and applies proportional auto-line spacing after the line.
      // Office fixed output exposes the same ownership as an invisible space
      // whose baseline is the drawing's bottom edge.
      inline_text_height(&base_style, text_metrics)
    } else {
...
fn ruby_line_height_for_flow(metrics: RubyMetrics, flow: FlowContext) -> f32 {
  if text_segment_uses_document_grid(flow.text_segmentation, flow.setup) {
    snap_line_height_to_doc_grid(metrics.height_pt, flow.setup.doc_grid_line_pitch_pt)
  } else {
    metrics.height_pt
  }
}
...
fn push_docx_picture_image(
  ...
  if let Some(frame) = image.picture_frame.as_deref() {
    ...
    if !clip_path.is_empty() {
      if inline_image_contains_reflection(image)
      ...
      let fill = inline_shape_common_fill(
        frame,
        Some(
          shape_transform
            * Affine::scale_non_uniform(
              f64::from(image_item.width_pt),
              f64::from(image_item.height_pt),
            )
            .then_translate((f64::from(image_item.x_pt), f64::from(image_item.y_pt)).into()),
        ),
      );
      let stroke = frame.stroke.map(|stroke| {
      ...
      if !matches!(fill, common::Fill::None) || stroke.is_some() {
...
fn inline_shape_fill_image_items(
  ...
  let make_item =
    |x_pt: f32, y_pt: f32, width_pt: f32, height_pt: f32, crop: ImageCrop| ImageItem {
      ...
      height_pt,
      inline_baseline_gap_pt: 0.0,
      inline_baseline_participant: matches!(shape.placement, crate::docx::ImagePlacement::Inline),
      crop,
...
struct RootFrameLayout<'a> {
  ...
  last_line_number_page_index: Option<usize>,
}
...
struct PageCheckpoint {
  ...
  floating_table_bounds_len: usize,
...
impl<'a> RootFrameLayout<'a> {
  fn new(document: &'a DocxDocument, options: &LayoutOptions) -> Self {
    Self {
      ...
      next_line_number: document
        .page
        .line_numbering
        .map(first_line_number)
        .unwrap_or(1),
      last_line_number_page_index: None,
    }
  }
  ...
  fn body_flow(&mut self, frame: BodyFrame) -> FlowContext {
    ...
    body_flow_for_page(
      FlowContext {
        ...
        do_not_expand_shift_return: self.document.do_not_expand_shift_return,
  ...
  fn record_layout_checkpoint(&mut self, block_index: usize, flow: FlowContext) {
    self.checkpoints.push(LayoutCheckpoint {
      ...
      last_line_number_page_index: self.last_line_number_page_index,
    });
  }
  ...
  fn format_block(
    ...
    let block_flow = footnote_boss_reserve(
      block,
      self.document,
      &self.emitted_footnotes,
      self.current.footnote_reserved_height_pt,
      *flow,
      &mut self.text_metrics,
    );
  ...
  fn add_line_numbers_for_block(
    ...
    if paragraph.format.suppress_line_numbers == Some(true) {
      // ECMA-376 Part 1 §17.3.1.35 removes the paragraph's lines from the
      // numbering calculation; it does not merely hide their labels.
      return;
    }
    ...
    let mut line_number_context = LineNumberPageContext {
      ...
      last_page_index: &mut self.last_line_number_page_index,
      text_metrics: &mut self.text_metrics,
    };
...
fn legacy_wordprocessing_effect_artifact(
  ...
  artifact.style.pdf_glyph_outlines = false;
...
fn materialize_wordprocessing_text_effects_in_items(
  ...
  for item in items {
    let text = match item {
      PageItem::Text(text) => text,
      PageItem::Group(items) | PageItem::FloatingDrawing { items, .. } => {
...
struct LineNumberPageContext<'a> {
  ...
  style: &'a TextStyle,
  content_left_pt: f32,
  next_number: &'a mut i32,
  last_page_index: &'a mut Option<usize>,
  text_metrics: &'a mut TextMetrics,
}

fn first_line_number(numbering: LineNumbering) -> i32 {
  i32::from(numbering.start) + 1
}

fn displays_line_number(number: i32, numbering: LineNumbering) -> bool {
  number.rem_euclid(i32::from(numbering.count_by)) == 0
}
...
fn add_line_numbers_to_page(
  ...
  if context.numbering.restart_each_page && *context.last_page_index != Some(page_index) {
    *context.next_number = first_line_number(context.numbering);
  }
  *context.last_page_index = Some(page_index);
  ...
  for line_box in line_boxes {
    ...
    *context.next_number = context.next_number.saturating_add(1);
    if !displays_line_number(number, context.numbering) {
      continue;
    }
...
fn line_number_text_metrics_for_items(
  ...
  items[item_start..item_end]
    ...
    .find_map(|item| match item {
      ...
      PageItem::Group(items) => line_number_text_metrics_for_items(items, 0, items.len()),
...
fn block_frame_influences(
  ...
  match block {
    Block::Paragraph(paragraph) => {
      let footnote_count = pending_block_footnote_ids(block, document, emitted_footnotes).len();
      if footnote_count > 0 {
        let height = referenced_footnote_area_height(
          block,
          document,
          emitted_footnotes,
          true,
...
fn normalize_layout_frames(frames: &mut Vec<LayoutFrame>, pages: &[Page]) {
  ...
  for frame in frames {
    // place_floating_images() detaches page-level drawings from their anchor
    // line. A mixed paragraph frame follows the remaining body items, while a
    // line made only from the detached anchor follows the drawing to the page
    // drawing layer. That line no longer owns an item inside the paragraph
    // frame and must not make the frame range span unrelated intervening
    // paragraphs. Frames made only from a floating object keep that object as
    // their range in PageItemOrder::remap_range(), so their records survive.
    frame.lines.retain(|line| {
      line.item_start >= frame.item_start
        && line.item_end <= frame.item_end
        && line.item_start < line.item_end
    });
    frame.fragments.retain(|fragment| {
...
fn item_line_y(item: &PageItem) -> Option<f32> {
  match item {
    ...
    PageItem::Group(items) => items.iter().find_map(item_line_y),
    PageItem::FloatingDrawing { .. } => None,
    PageItem::Image(_)
    | PageItem::Rect(_)
    | PageItem::Fill(_)
...
fn item_bounds(item: &PageItem, text_metrics: &mut TextMetrics) -> Option<(f32, f32, f32, f32)> {
  match item {
    ...
    PageItem::Group(items) | PageItem::FloatingDrawing { items, .. } => {
      page_items_bounds(items, text_metrics)
    }
...
fn page_checkpoint(page: &Page) -> PageCheckpoint {
  PageCheckpoint {
    ...
    floating_table_bounds_len: page.floating_table_bounds.len(),
...
fn restore_page_checkpoint(page: &mut Page, checkpoint: PageCheckpoint) {
  ...
  page
    ...
    .truncate(checkpoint.footnote_item_indices_len);
  ...
  page
    .floating_table_bounds
    .truncate(checkpoint.floating_table_bounds_len);
  page
...
fn estimated_paragraph_content_height(
  ...
  let mut line_height =
    include_numbering_label_height(paragraph, base_line_height, text_frame, text_metrics);
  ...
  for item in &paragraph.inlines {
    match item {
      ...
      InlineItem::Ruby(ruby) => {
        ...
        let ruby_height = ruby_line_height_for_flow(metrics, flow);
        has_flow_content = true;
      ...
      InlineItem::Shape(shape) => {
        ...
        line_height = if flow.compatibility_mode < 15 && compatibility_forced_shape_line {
          frame_height.max(LAYOUT_EPSILON_PT)
        } else {
          line_height.max(frame_height)
        };
        x = frame_width;
      }
      InlineItem::FormWidgetStart(_)
...
fn layout_document_block(
  ...
  match block {
    Block::Paragraph(paragraph) => {
      ...
      // frame-relative orientation. Its positioning base starts at the
      // paragraph frame, before the printable text area's upper spacing.
...
fn floating_frame_width(
  ...
  // SizeType::MIN frame, allowing its fly to grow around the content.
  frame
    .blocks
    .iter()
...
fn note_separator_layout_height(kind: NoteSeparatorKind) -> f32 {
  LO_FOOTNOTE_SEPARATOR_TOP_DIST_PT
    + match kind {
      NoteSeparatorKind::Footnote => LO_FOOTNOTE_SEPARATOR_BOTTOM_DIST_PT,
      NoteSeparatorKind::Endnote | NoteSeparatorKind::EndnoteContinuation => {
        LO_ENDNOTE_SEPARATOR_BOTTOM_DIST_PT
...
fn referenced_footnote_area_height(
  ...
  let mut height = if include_separator && !ids.is_empty() {
    note_separator_layout_height(NoteSeparatorKind::Footnote)
  } else {
    0.0
  };
  for id in ids {
    height += measured_note_blocks_height(&document.footnotes[&id], flow, text_metrics);
  }
  height
}
...
fn footnote_boss_format(
  ...
  let new_height = ids
    .iter()
    .map(|id| measured_note_blocks_height(&document.footnotes[id], flow, target.text_metrics))
    .sum::<f32>()
    + if needs_separator {
      note_separator_layout_height(NoteSeparatorKind::Footnote)
    } else {
...
fn record_page_footnote_items(
  ...
  } else if let Some(page) = pages.get_mut(start_pages_len) {
    page
      .footnote_item_indices
      .extend(start_item_index..page.items.len());
  }
}
...
fn block_has_visible_body_content(block: &Block) -> bool {
  match block {
    Block::Paragraph(paragraph) => paragraph.inlines.iter().any(|inline| match inline {
      ...
      InlineItem::Ruby(_) | InlineItem::Image(_) | InlineItem::Shape(_) => true,
...
fn wrap_floating_page_item_range(
  ...
  if item_start >= item_end || text_segmentation == TextSegmentation::TableCell {
...
fn resolve_dynamic_fields(
  ...
  for (page_index, page) in pages.iter_mut().enumerate() {
    ...
    for item in &mut page.items {
      ...
      resolve_dynamic_field_text(text, &context, false, text_metrics);
    }
    if let Some(adornment) = &mut page.repeating_adornment {
      for item in &mut adornment.items {
        let PageItem::Text(text) = item else {
          continue;
        };
        resolve_dynamic_field_text(text, &context, true, text_metrics);
      }
    }
  ...
  match &text.dynamic_field {
    ...
    Some(DynamicFieldKind::PageRef {
      ...
      if let Some(anchor) = context.page_refs.get(bookmark_name.as_ref()) {
        ...
        text.text = if !relative_position {
          ...
          localized_pageref_on_page(context.ui_language, &page_number)
        } else {
          ...
          localized_pageref_relative(context.ui_language, bookmark_is_above)
        };
      } else {
        text.text = localized_undefined_bookmark(context.ui_language);
        super::apply_generated_field_error_font(&mut text.style, context.ui_language);
    ...
    Some(DynamicFieldKind::StyleRef {
      ...
      if let Some(value) = resolve_style_ref(
        ...
        text.text = localized_missing_style(context.ui_language, style_name);
        apply_generated_style_ref_error_font(&mut text.style, context.ui_language);
      }
    }
    None => {}
  }
...
fn localized_missing_style(ui_language: Option<&str>, style_name: &str) -> String {
  let language = normalized_field_language(ui_language);
  if language == "zh-cn" || language == "zh-sg" || language.starts_with("zh-hans") {
...
fn apply_generated_style_ref_error_font(style: &mut TextStyle, ui_language: Option<&str>) {
  let language = normalized_field_language(ui_language);
  if language == "zh-cn" || language == "zh-sg" || language.starts_with("zh-hans") {
    // Word's localized STYLEREF diagnostic is a bold UI resource. Unlike the
    // missing-bookmark resource above, it uses the current Office UI face for
    // Chinese while retaining the field's Latin face for the embedded style
    // name and punctuation.
    style.east_asia_font_family = Some(super::office_default_font_family(ui_language));
    style.east_asia_language = Some(Arc::<str>::from("zh-CN"));
    style.bold = true;
    style.complex_bold = Some(true);
    style.line_height_override_pt =
      Some(style.font_size_pt * super::WORD_ZH_STYLE_REF_ERROR_LINE_HEIGHT_PER_FONT_SIZE);
  }
}
...
fn resize_dynamic_field_tab_leaders(items: &mut [PageItem], text_metrics: &mut TextMetrics) {
  ...
  for (field_index, item) in items.iter().enumerate() {
    ...
    let repeat_width = tab_leader_repeat_width(fill_char, &leader.style, text_metrics);
    if repeat_width <= LAYOUT_EPSILON_PT {
      continue;
    }
    let anchor = field
    ...
    let x_pt = match leader_layout {
      ...
      TabLeaderLayout::OfficeLegacyPrintGridEnd | TabLeaderLayout::Office2013PrintGridEnd => {
        aligned_content_x - count as f32 * repeat_width
      }
    };
...
fn layout_floating_table(
  ...
  let (requested_x, mut requested_frame_y) =
    floating_table_position(placement, flow, y, table_width);
  if !table.split_allowed {
    // ECMA-376 Part 4 §14.8.3.10 says an unsplittable wrapped table stays on
    // its first page by moving its start upward as needed. Clamp to the
    // content top when the table itself is taller than the available region;
    // the extended frame bounds below then keep that oversized fly whole.
    requested_frame_y = unsplit_floating_table_start_y(
      requested_frame_y,
      layout.frame.total_height,
      flow.content_top_pt,
      flow.content_bottom,
    );
  }
  ...
  // Writer resolves tblOverlap against the outer fly frame. When a default
  // text-anchored table is pushed below an earlier fly, Word's displayed
  // table content starts after the outer frame's leftFromText distance. Keep
  ...
  let frame_flow = FlowContext {
    content_top_pt: frame_y,
    content_left_pt: x,
    content_bottom: if table.split_allowed {
      flow.content_bottom
    } else {
      unsplit_content_bottom
    },
    body_content_bottom_pt: if table.split_allowed {
      flow.body_content_bottom_pt
    } else {
      unsplit_content_bottom
    },
  ...
  for (page_index, page) in frame_pages.iter_mut().enumerate() {
    ...
    if let Some(bounds) = floating_table_frame_bounds(page, text_metrics) {
  ...
  if let Some(first_page) = frame_pages.first_mut() {
    ...
    current
      .floating_table_bounds
      .append(&mut first_page.floating_table_bounds);
    current
      .pending_floating_table_follows
...
fn page_item_is_path(item: &PageItem) -> bool {
  match item {
    PageItem::Group(items) | PageItem::FloatingDrawing { items, .. } => {
      items.iter().any(page_item_is_path)
    }
...
fn append_floating_table_wrap_exclusion(
  ...
  let Some(bounds) = floating_table_frame_bounds(page, text_metrics) else {
    return;
  };
  let left_pt = bounds.x_pt;
  let top_pt = bounds.y_pt;
  let right_pt = bounds.x_pt + bounds.width_pt;
  let bottom_pt = bounds.y_pt + bounds.height_pt;
...
fn floating_table_frame_bounds(page: &Page, text_metrics: &mut TextMetrics) -> Option<FrameBounds> {
  ...
  table_bounds
    .or_else(|| page_items_bounds(&page.items, text_metrics))
    .map(|(left_pt, top_pt, right_pt, bottom_pt)| FrameBounds {
      x_pt: left_pt,
      y_pt: top_pt,
      width_pt: (right_pt - left_pt).max(0.0),
      height_pt: (bottom_pt - top_pt).max(0.0),
    })
}
...
fn translate_page_item(mut item: PageItem, dx_pt: f32, dy_pt: f32) -> PageItem {
  match &mut item {
    ...
    PageItem::Image(image) => {
      image.x_pt += dx_pt;
      image.y_pt += dy_pt;
      translate_image_clip_path(image, dx_pt, dy_pt);
    }
    PageItem::Rect(rect) => {
      rect.x_pt += dx_pt;
      rect.y_pt += dy_pt;
  ...
  items.extend(
    lower_clustered_column_chart(
      ...
      &ClusteredColumnStyle {
        ...
        series_gradient_fills: chart.series_gradient_fills.clone(),
  ...
  fn format(
    ...
    'table_rows: while row_index < layout.table.rows.len() {
      ...
      if split_decision.split_row_allowed {
        ...
        let mut cell_content_cursors: Option<Vec<f32>> = None;
        while remaining_height > LAYOUT_EPSILON_PT {
          ...
          let cell_cursors = layout
            ...
            .format_fragment_with_cell_cursors(
              ...
              y + fragment_height,
              content_offset,
              cell_content_cursors.as_deref(),
            );
          if layout.table.placement.is_some() {
            let cursors =
              cell_content_cursors.get_or_insert_with(|| vec![content_offset; cell_cursors.len()]);
            if cursors.len() < cell_cursors.len() {
              cursors.resize(cell_cursors.len(), content_offset);
            }
            for (index, cursor) in cell_cursors.into_iter().enumerate() {
              if let Some(cursor) = cursor {
                cursors[index] = cursor;
              } else {
                cursors[index] += fragment_height;
              }
            }
          }
          extend_border_segment(
          ...
          if remaining_height > LAYOUT_EPSILON_PT {
            // A split floating-table follow remains a fragment of the same
            // at-least-height row. Writer's testSplitFlyWidow exposes the
            // authored w:trHeight on the follow row as well (1029 twips after
            // border accounting), rather than shrinking it to the arithmetic
            // tail left after the master fragment.
            remaining_height = remaining_height.max(min_split_height);
          }
...
impl RowFrame<'_, '_> {
  ...
  fn format_fragment_with_cell_cursors(
    ...
    row_bottom: f32,
    content_offset: f32,
    cell_content_cursors: Option<&[f32]>,
  ) -> Vec<Option<f32>> {
  ...
  fn leading_border_segment(&self, row_top: f32, row_bottom: f32) -> Option<PendingBorderSegment> {
    ...
    let border = vertical_border(self.table, self.row, 0, true)
      ...
      .or_else(|| row_table_borders(self.table, self.row).and_then(|borders| borders.left))?;
  ...
  fn trailing_border_segment(&self, row_top: f32, row_bottom: f32) -> Option<PendingBorderSegment> {
    let border = self
      ...
      .or_else(|| row_table_borders(self.table, self.row).and_then(|borders| borders.right))?;
    Some(PendingBorderSegment {
...
fn table_column_widths(
  ...
  let widths = fixed_table_column_widths(table, column_count, content_width, allow_width_overflow);
  if !table
    .rows
    .iter()
  ...
  autofit_table_column_widths(table, widths, content_width, text_metrics)
}
...
fn fixed_table_column_widths(
  ...
  if table.column_widths_pt.len() >= column_count {
    ...
    let fixed_grid_expands_preferred_width = table.layout == TableLayoutMode::Fixed
      && preferred_width.is_some_and(|preferred| grid_width > preferred + LAYOUT_EPSILON_PT);
    if !fixed_grid_expands_preferred_width
    ...
    // not a maximum. Otherwise an explicit table preferred width constrains
    // and proportionally scales those columns.
    if !allow_width_overflow && preferred_width.is_some() && !fixed_grid_expands_preferred_width {
...
fn autofit_table_column_widths(
  ...
  for row in &table.rows {
    ...
    for cell in &row.cells {
      ...
      saw_content |= maximum > margins + LAYOUT_EPSILON_PT;
  ...
  for (start, span, minimum) in constraints {
    ...
    let mut deficit = minimum - current;
    ...
    grow_spanned_columns_to_width(&mut widths[start..end], current + reclaimed);
    deficit -= reclaimed;
...
fn block_content_width_range(
  ...
  match block {
    ...
    Block::Table(table) => {
      ...
      let widths = fixed_table_column_widths(table, column_count, content_width, true);
...
fn layout_table_cell(fragment: TableCellLayout<'_>) -> Option<f32> {
  ...
  if has_following_text_flow_table && !split_fragment {
    ...
    if let Some(mut first_page) = local_pages.next() {
      ...
      current
        .items
        .extend(first_page.items.into_iter().filter(|item| {
  ...
  // SwTextFrame follows continue from a text cursor, not from the physical
  // height of the split row. Record each floating cell's cumulative content
  // cursor from the paragraph-line fragments that this row fragment owns.
  // This also preserves independent cursors for cells with different line
  // heights (SwTextFrame::Prepare sets the follow offset to aLine.GetEnd()).
  let content_cursor = word_floating_table_cell
    .then(|| floating_table_cell_content_cursor(&nested_fragments, y, text_bottom, content_start_y))
    .flatten();
  let item_start = current.items.len();
  current
    .items
...
fn table_cell_item_intersects_vertical_bounds(item: &PageItem, top: f32, bottom: f32) -> bool {
  match item {
    PageItem::Group(items) | PageItem::FloatingDrawing { items, .. } => items
      .iter()
...
fn floating_table_cell_content_cursor(
  fragments: &[FrameFragment],
  top: f32,
  bottom: f32,
  content_start_y: f32,
  ...
  fragments
    ...
    .filter_map(|fragment| {
      let bounds = fragment.bounds?;
      table_cell_frame_fragment_intersects_vertical_bounds(fragment, top, bottom)
        .then_some((bounds.y_pt + bounds.height_pt - content_start_y).max(0.0))
    })
    .reduce(f32::max)
...
fn floating_image_position(
  ...
  let (base_y, reference_height) = match vertical_reference {
        ...
        ) =>
    {
      ...
      // nested table flow stores that line relative to the page text area;
      // preserve that offset when the paragraph/line reference is promoted
      // to a page reference. LibreOffice models the same ownership by setting
      // IsFollowingTextFlow=false in GraphicImport.cxx.
      (
        flow.content_top_pt - flow.setup.margin_top_pt,
        flow.setup.height_pt,
      )
    }
...
impl TextFrame {
  fn new(
    ...
    Self {
      ...
      paragraph_indent_right_pt: indent_right_pt,
      base_line_height,
      auto_baseline_line_height_cap: generated_resource_baseline_cap.or_else(|| {
        (base_line_height > available_frame_height + LAYOUT_EPSILON_PT)
          .then(|| {
            paragraph_auto_baseline_line_height_cap(paragraph, &base_line_style, text_metrics)
          })
          .flatten()
      }),
      line_height_rule: paragraph.format.line_height_rule,
...
fn can_defer_page_break_for_following_floating_anchor(
  ...
  let Some(anchor_index) = paragraph
    ...
    .find_map(|(index, inline)| match inline {
      ...
      InlineItem::PositionalTab(_) => None,
      InlineItem::Ruby(_) => Some(usize::MAX),
      InlineItem::BookmarkStart(_)
      | InlineItem::FormWidgetStart(_)
      | InlineItem::FormWidgetEnd(_)
      | InlineItem::DrawingGroupStart(_)
...
fn page_break_has_following_content(inlines: &[InlineItem], next_inline_index: usize) -> bool {
  inlines
    .iter()
    .skip(next_inline_index)
    .find_map(|inline| match inline {
      ...
      InlineItem::Text(_) | InlineItem::Ruby(_) | InlineItem::Image(_) | InlineItem::Shape(_) => {
        Some(true)
      }
    })
    .unwrap_or(false)
...
fn justify_soft_break_line(justify_wrapped_lines: bool, do_not_expand_shift_return: bool) -> bool {
  // ECMA-376 Part 1 §17.15.3.5 keeps a soft-break line in block
  // justification by default. w:doNotExpandShiftReturn is the opt-out.
  justify_wrapped_lines && !do_not_expand_shift_return
}
...
impl<'a> TextFrameLayout<'a> {
  ...
  fn advance_line(
    ...
    let real_height = line_real_height(
      ...
      *line_height,
      ...
      advance.active.frame.grid_auto_following_line_height_pt,
  ...
  fn force_text_page_break(
    ...
    if preserve_paragraph_upper_space {
      // An explicit w:br page break can split one paragraph into two page
      // frames. Word/Writer's PARA_SPACE_MAX_AT_PAGES compatibility keeps the
      // paragraph's upper spacing on the follow when more content remains in
      // that same paragraph (tdf#160049). Natural pagination and a terminal
      // page break do not gain this spacing.
      y += paragraph_upper_space(self.paragraph, text_metrics);
    }
  ...
  fn format_with_reflow(
    ...
    let start_flow = flow;
    let paragraph_top = y;
    // Paragraph-relative drawings use the paragraph frame origin supplied by
    // the owning block. Only line-relative drawings need the table-cell
    // baseline translated back to a SwLineLayout top (see
    // `floating_anchor_line_top`). In particular, bottom-aligned table
    // paragraphs keep their aligned frame origin here.
    ...
    let mut line_height =
      include_numbering_label_height(paragraph, line.height_pt, text_frame, text_metrics);
    ...
    if let Some(label) = &paragraph.list_label {
      ...
      let label_x = if blank_list_label {
        default_line_left
      } else {
    ...
    for (inline_index, item) in paragraph.inlines.iter().enumerate() {
      match item {
        ...
        InlineItem::Ruby(ruby) => {
          ...
          let metrics = ruby_inline_metrics(ruby, text_metrics);
          if text_overflows_line(x, metrics.width_pt, line_right)
            && x > line_left
            && pending_tab.is_none()
          ...
          if !matches!(text_frame.line_height_rule, LineHeightRule::Exact) {
            line_height = line_height.max(ruby_line_height_for_flow(metrics, flow));
          }
          push_ruby_items(
            current,
            paragraph,
            ...
            let selected_hyphenation = (overflows_line
              ...
              .then(|| {
                select_hyphenation_break(
                  &source_segment,
                  &run.style,
                  (line_right - x - leading_script_spacing).max(0.0),
        ...
        InlineItem::Shape(shape) => {
          ...
          match shape.placement {
            crate::docx::ImagePlacement::Floating(placement) => {
              ...
              let authored_width =
                relative_floating_width(placement, flow).unwrap_or(shape.width_pt);
...
fn tab_leader_repeat_count(
  ...
  let (usable_width_pt, repeat_width_pt) = match layout {
    ...
    // Word 2013+ compatibility layout counts aligned-tab leaders with the
    // fixed-output printer advance. For 11pt Calibri, a natural 2.776855pt
...
fn tab_leader_style(
  ...
  if !synthetic_bold && style.fallback_font_family.is_some() {
    // Office emits printer-substituted legacy leaders on whole 600dpi
    // advances. Adjust tracking on the resolved face so both shaping and the
    // fixed-output stream use the same advance (the Calibri fallback in
    // testPageref.docx resolves from 3.0293pt to 3.0pt).
...
fn push_tab_leader(
  ...
  let style = tab_leader_style(fill_char, style, text_metrics);
  let repeat_width = tab_leader_repeat_width(fill_char, &style, text_metrics);
  if repeat_width <= LAYOUT_EPSILON_PT {
    return;
  }
  let boundary_slot = tab_leader_boundary_slot_pt(
  ...
  let x_pt = match placement.layout {
    ...
    TabLeaderLayout::OfficeLegacyPrintGridEnd | TabLeaderLayout::Office2013PrintGridEnd => {
      placement.end_x - count as f32 * repeat_width
    }
  };
...
fn item_y(item: &PageItem) -> Option<f32> {
  match item {
    ...
    PageItem::Group(items) => items.iter().find_map(item_y),
    PageItem::FloatingDrawing { .. } => None,
...
fn inline_image_metrics(image: &crate::docx::InlineImage, max_width_pt: f32) -> InlineImageMetrics {
  ...
  InlineImageMetrics {
    ...
    content_bottom_gap_pt: image.effect_bottom_pt.max(0.0) * scale,
  }
}
...
mod tests {
  ...
  fn body_style_ref_searches_up_before_down_and_respects_localized_style_names() {
    ...
    assert_eq!(
      localized_missing_style(Some("zh-CN"), "Heading 1"),
  ...
  fn ruby_compound_height_advances_to_next_document_grid_multiple() {
    ...
    assert_eq!(ruby_line_height_for_flow(metrics, flow), 36.0);
  }
  ...
  fn automatic_before_spacing_overrides_authored_spacing_and_is_suppressed_at_first_body_paragraph()
  {
    ...
    assert_eq!(
      paragraph_upper_space(&paragraph, &mut text_metrics),
    ...
    assert_eq!(
      paragraph_spacing_before(None, &paragraph, flow, &mut text_metrics),
      0.0
    );
    assert_eq!(
  ...
  fn page_owned_cell_floating_uses_the_table_anchor_line_offset() {
    ...
    let flow = FlowContext {
      ...
      ..flow_from_block_area(BlockArea {
        ...
        columns: SectionColumns::default(),
        content_top_pt: 103.0,
        content_left_pt: 90.0,
        content_bottom: 140.0,
  ...
  fn numbering_label_font_contributes_to_the_first_line_height() {
    ...
    let height =
      include_numbering_label_height(&paragraph, frame.base_line_height, frame, &mut text_metrics);
  ...
  #[test]
  fn floating_table_cell_cursor_uses_owned_line_baselines() {
    ...
    assert_eq!(
      floating_table_cell_content_cursor(&fragments, 50.0, 76.0, 40.0),
      Some(36.0)
    );
  }
  ...
  #[test]
  fn grid_auto_line_spacing_uses_one_grid_pitch_as_the_proportional_base() {
    let mut paragraph = Paragraph {
      inlines: Vec::new(),
      field_events: Vec::new(),
      footnote_reference_ids: Vec::new(),
  ...
  fn only_inline_object_images_set_the_text_line_baseline() {
    ...
    align_line_items_to_inline_object_baseline(&mut floating_backdrop, 0, 0.0, &mut text_metrics);
    let PageItem::Text(text) = &floating_backdrop[1] else {
      panic!("expected text item");
    };
    assert_eq!(text.y_pt, 0.0);

    align_line_items_to_inline_object_baseline(&mut items, 0, 0.0, &mut text_metrics);
  ...
  #[test]
  fn table_cell_floating_drawing_stays_in_its_text_frame_sequence() {
    let placement = FloatingImagePlacement {
      horizontal_relative_to: HorizontalImageReference::Column,
      vertical_relative_to: VerticalImageReference::Paragraph,
      horizontal_alignment: None,
