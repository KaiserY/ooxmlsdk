...
pub(crate) fn shape_text_runs(text: &str, style: &TextStyle) -> Option<Vec<ShapedRun<'static>>> {
...
pub(crate) fn font_face_data(font_id: &FontId) -> Option<FontFaceData> {
  font_data_cache().lock().ok()?.get(font_id).cloned()
}

pub(crate) fn vertical_metrics(style: &TextStyle) -> Option<ooxmlsdk_fonts::VerticalMetrics> {
  let request = font_request(style);
  let registry = style_font_registry(style, None);
  let resolved = registry.resolve(&request).ok()?;
  Some(
...
pub(crate) fn decoration_metrics(style: &TextStyle) -> Option<ooxmlsdk_fonts::DecorationMetrics> {
  let request = font_request(style);
  let registry = style_font_registry(style, None);
  let resolved = registry.resolve(&request).ok()?;
  Some(
...
pub(crate) fn cached_text_face(style: &TextStyle) -> Option<FontFaceData> {
  let key = FontFaceKey {
    family: style.font_family.as_deref().map(str::to_string),
    bold: style.bold,
    italic: style.italic,
