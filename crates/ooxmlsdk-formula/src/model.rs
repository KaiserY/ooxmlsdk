...
fn parse_formula<'doc>(
  ...
  let body_start = crate::parser::formula_body_start(source.as_ref());
  parse_formula_legacy(sheet, source, grammar, body_start)
}

pub(crate) fn parse_formula_legacy<'doc>(
  sheet: SheetId,
  source: Cow<'doc, str>,
  grammar: FormulaGrammar,
  body_start: usize,
  ...
  for token in crate::parser::semantic_tokens(text) {
    ...
    match token.kind {
      ...
      crate::parser::SemanticTokenKind::Error(error) => {
        tokens.push(FormulaToken::Literal(FormulaValue::Error(
          formula_error_from_lex(error),
        )));
      }
      crate::parser::SemanticTokenKind::Operator(operator) => {
        tokens.push(FormulaToken::Operator(formula_operator_from_lex(operator)));
      }
      crate::parser::SemanticTokenKind::ArrayOpen => {
        tokens.push(FormulaToken::ArrayOpen);
      }
      crate::parser::SemanticTokenKind::ArrayClose => {
        tokens.push(FormulaToken::ArrayClose);
      }
      crate::parser::SemanticTokenKind::ArgumentSeparator => {
        tokens.push(FormulaToken::Separator(FormulaSeparator::Argument));
      }
      crate::parser::SemanticTokenKind::RowSeparator => {
        tokens.push(FormulaToken::Separator(FormulaSeparator::Row));
      }
      crate::parser::SemanticTokenKind::ParenOpen
      | crate::parser::SemanticTokenKind::ParenClose => {}
      crate::parser::SemanticTokenKind::Function { volatile } => {
        let word = lexeme;
        if volatile {
      ...
      crate::parser::SemanticTokenKind::Boolean(value) => {
        tokens.push(FormulaToken::Literal(FormulaValue::Boolean(value)));
      }
      crate::parser::SemanticTokenKind::ExternalReference(reference) => {
        let external = external_reference_id_from_spans(text, reference);
...
fn parse_formula_ast<'doc>(
  ...
  let syntax = crate::parser::parse_syntax_ast(text);
  let mut unsupported = Vec::new();
  for issue in syntax.issues {
    match issue {
      crate::parser::SyntaxIssue::MissingClosingParenthesis => {
        unsupported.push(UnsupportedFormulaFeature {
  ...
  let ast = syntax
    .ast
    .as_ref()
    .and_then(|ast| formula_ast_from_syntax(sheet, text, ast));
  if ast.is_some() && syntax.complete {
    (ast, unsupported)
  } else {
    unsupported.push(UnsupportedFormulaFeature {
...
fn formula_ast_from_syntax<'doc>(
  sheet: SheetId,
  text: &str,
  ast: &crate::parser::SyntaxNode,
) -> Option<FormulaAst<'doc>> {
  match ast {
    ...
    crate::parser::SyntaxNode::Error(error) => Some(FormulaAst::Literal(FormulaValue::Error(
      formula_error_from_lex(*error),
    ))),
    crate::parser::SyntaxNode::Word(span) => formula_ast_from_syntax_word(sheet, text, *span),
    crate::parser::SyntaxNode::Unary { op, expr } => {
      let op = match op {
      ...
      Some(FormulaAst::Unary {
        op,
        expr: Box::new(formula_ast_from_syntax(sheet, text, expr)?),
      })
    }
    crate::parser::SyntaxNode::Binary { op, left, right } => Some(FormulaAst::Binary {
      op: formula_operator_from_lex(*op),
      left: Box::new(formula_ast_from_syntax(sheet, text, left)?),
      right: Box::new(formula_ast_from_syntax(sheet, text, right)?),
    }),
    crate::parser::SyntaxNode::Function { name, args } => Some(FormulaAst::Function {
      name: Cow::Owned(text.get(name.start..name.end)?.to_string()),
      args: formula_ast_args_from_syntax(sheet, text, args)?,
    }),
    crate::parser::SyntaxNode::LogicalFunction { function, args } => Some(FormulaAst::Function {
      name: Cow::Borrowed(function.name()),
      args: formula_ast_args_from_syntax(sheet, text, args)?,
    }),
    crate::parser::SyntaxNode::Array(rows) => Some(FormulaAst::Array(
      rows
        .iter()
        .map(|row| formula_ast_args_from_syntax(sheet, text, row))
        .collect::<Option<Vec<_>>>()?,
    )),
  }
}

fn formula_ast_args_from_syntax<'doc>(
  sheet: SheetId,
  text: &str,
  args: &[crate::parser::SyntaxNode],
) -> Option<Vec<FormulaAst<'doc>>> {
  args
    .iter()
    .map(|arg| formula_ast_from_syntax(sheet, text, arg))
    .collect()
}

fn formula_ast_from_syntax_word<'doc>(
  sheet: SheetId,
  text: &str,
...
impl<'a, 'doc> FormulaEvaluator<'a, 'doc> {
  ...
  fn evaluate_function(
    ...
    match upper.as_str() {
      ...
      "ERRORTYPE" => self.evaluate_error_type_raw(args),
      ...
      "DEGREES" => args
        ...
        .or(Some(FormulaValue::Error(FormulaErrorValue::Unknown))),
      "SIN" => self.evaluate_numeric_unary(args, rtl_sin),
      "CSC" => self.evaluate_numeric_unary(args, |value| 1.0 / rtl_sin(value)),
      "COS" => self.evaluate_numeric_unary(args, rtl_cos),
      "SEC" => self.evaluate_numeric_unary(args, |value| 1.0 / rtl_cos(value)),
      "TAN" => self.evaluate_numeric_unary(args, rtl_tan),
      "COT" => self.evaluate_numeric_unary(args, |value| 1.0 / rtl_tan(value)),
      ...
      "SUMSQ" => match self.numeric_aggregate(args, true) {
      ...
      "T" => self.evaluate_t(args),
      "VALUE" => self.evaluate_value(args),
      ...
      "CONVERT" | "CONVERT_OOO" => self.evaluate_convert(args),
      ...
      "CLEAN" => self.evaluate_clean(args),
      ...
      "FINDB" => self.evaluate_findb(args, true),
      "SEARCH" => self.evaluate_find(args, false),
      "SEARCHB" => self.evaluate_findb(args, false),
      "REPT" => {
        let text = self.text(&self.evaluate(args.first()?)?);
        let count = self.number(&self.evaluate(args.get(1)?)?)?;
        if count < 0.0 {
          return Some(FormulaValue::Error(FormulaErrorValue::IllegalArgument));
        }
      ...
      "STDEV.P" | "STDEVP" => match self.st_var_numbers(args, true) {
        Ok(values) => {
          variance_slice(&values, false).map(|value| FormulaValue::Number(value.sqrt()))
        }
        Err(error) => Some(FormulaValue::Error(error)),
      },
      "STDEV.S" => match self.st_var_numbers(args, true) {
      ...
      "GAUSS" => Some(FormulaValue::Number(lo_gauss(
      ...
      "PHI" => Some(FormulaValue::Number(lo_phi(
        self.number(&self.evaluate(args.first()?)?)?,
      ))),
      ...
      "F.INV" | "FINV" => self.evaluate_f_inv(args, false),
      "F.INV.RT" | "LEGACY.FINV" => self.evaluate_f_inv(args, true),
      ...
      "MODE.MULT" => self.evaluate_mode_ms(args, false),
      "MODE.SNGL" => self.evaluate_mode_ms(args, true),
      "MODE" => self.evaluate_mode(args),
      "NEGBINOM.DIST" => self.evaluate_negbinom_dist(args, true),
      "NEGBINOMDIST" => self.evaluate_negbinom_dist(args, false),
      ...
      "POISSON.DIST" => self.evaluate_poisson_dist(args, false),
      "POISSON" => self.evaluate_poisson_dist(args, true),
      ...
      "TDIST" | "LEGACY.TDIST" => self.evaluate_t_dist_legacy(args),
      ...
      "ORG.OPENOFFICE.ROT13" => self.evaluate_rot13(args),
  ...
  fn evaluate_clean(&self, args: &[FormulaAst<'doc>]) -> Option<FormulaValue<'doc>> {
  ...
  fn evaluate_trim(&self, args: &[FormulaAst<'doc>]) -> Option<FormulaValue<'doc>> {
    let value = self.evaluate(args.first()?)?;
    if self.array_context && matches!(value, FormulaValue::Reference(_) | FormulaValue::Matrix(_)) {
      return Some(FormulaValue::Matrix(
        self
  ...
  fn evaluate_t(&self, args: &[FormulaAst<'doc>]) -> Option<FormulaValue<'doc>> {
  ...
  fn evaluate_value(&self, args: &[FormulaAst<'doc>]) -> Option<FormulaValue<'doc>> {
  ...
  fn value_from_formatter_text(&self, value: &FormulaValue<'doc>) -> FormulaValue<'doc> {
    match value {
  ...
  fn evaluate_mod(&self, args: &[FormulaAst<'doc>]) -> Option<FormulaValue<'doc>> {
    if args.len() != 2 {
      return Some(FormulaValue::Error(FormulaErrorValue::Unknown));
    }
  ...
  fn sheets_count_from_ast(&self, arg: &FormulaAst<'doc>) -> Option<u32> {
  ...
  fn reference_sheet_count(&self, reference: &QualifiedRange<'doc>) -> u32 {
  ...
  fn three_d_reference_sheet_count(&self, text: &str) -> Option<u32> {
  ...
  fn evaluate_mode(&self, args: &[FormulaAst<'doc>]) -> Option<FormulaValue<'doc>> {
    let values = self.numeric_args(args);
    mode_slice(&values)
      .map(FormulaValue::Number)
      .or(Some(FormulaValue::Error(FormulaErrorValue::NA)))
  ...
  fn evaluate_t_dist_legacy(&self, args: &[FormulaAst<'doc>]) -> Option<FormulaValue<'doc>> {
    if args.len() != 3 {
      return Some(FormulaValue::Error(FormulaErrorValue::Parameter));
    }
    let t = self.number(&self.evaluate(args.first()?)?)?;
  ...
  fn collect_st_var_numbers_from_ast(
    &self,
    ast: &FormulaAst<'doc>,
    direct_text_error: bool,
    values: &mut Vec<f64>,
  ...
  fn mode_ms_numeric_args(&self, args: &[FormulaAst<'doc>]) -> Vec<f64> {
    let mut values = Vec::new();
    let array_evaluator = self.with_array_context();
    for arg in args {
      let ranges = self.reference_ranges_from_ast(arg);
  ...
  fn push_range_numbers_column_major(
    &self,
    reference: &QualifiedRange<'doc>,
    values: &mut Vec<f64>,
  ) {
...
fn external_reference_id_from_spans<'doc>(
  ...
  ExternalReferenceId {
    book: Some(Cow::Owned(span_text(source, reference.book).to_string())),
    sheet: reference
      .sheet
      .map(|sheet| Cow::Owned(span_text(source, sheet).replace("''", "'"))),
    name: reference
      .name
      .map(|name| Cow::Owned(span_text(source, name).to_string())),
  }
}
